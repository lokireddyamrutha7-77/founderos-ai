...
export const api = {
  auth: {
    async signup(email: string, name: string, password?: string, businessName?: string): Promise<User> {
      const data = await fetchAPI('/auth/signup', {
        method: 'POST',
        body: JSON.stringify({ email, name, password: password || 'default_password' })
      });
      return data;
    },
